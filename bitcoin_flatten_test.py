...
for input in payload_dict["inputs"]:
    for output in payload_dict["outputs"]:
        if not payload_dict["is_coinbase"]:
                if output["value"] > 0:
                    token_outgoing_value = round((1e-8 * input["value"]) * (1e-8 * output["value"]) / (1e-8 * payload_dict["output_value"]),8)
                else:
                    token_outgoing_value = round((1e-8 * input["value"]) / payload_dict["output_count"],8)
                if input["value"] > 0:
                    token_incoming_value = round((1e-8 * input["value"]) * (1e-8 * output["value"]) / (1e-8 * payload_dict["input_value"]),8)
                else:
                    token_incoming_value = 0

                token_outgoing_fee = round(token_outgoing_value - token_incoming_value,9)
